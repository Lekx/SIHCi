...
<?php $this->widget('zii.widgets.CDetailView', array(
	...
	//	'id',
		//'id_user_sponsorer',
			  array(
			   'label'=>'Empresa que patrocina',
			   'value'=>Sponsors::model()->findByAttributes(array("id_user"=>$model->id_user_sponsorer))->sponsor_name,
			   'name'=>'id_user_sponsorer',
			   ),
			  array(
			   'label'=>'Investigador a patrocinar',
			   'value'=>$model->idUserResearcher->persons[0]["last_name1"]." ".$model->idUserResearcher->persons[0]["last_name2"]." ".$model->idUserResearcher->persons[0]["names"],
...
//echo $model->id;
$sponpro = SponsoredProjects::model()->findByAttributes(array("id_sponsorship"=>$model->id));
//var_dump($sponpro);

if(is_object($sponpro)){

	echo "Detalles del proyecto aceptado.<hr>";

	$model = Projects::model()->findByPk($sponpro->id_project);
...
if($model->status !="BORRADOR"){
	...
	echo "Usted podrá generar comentarios acerca del proyecto una vez que el investigador lo envie a evaluación.";
}
?>
</div>
