...
class SponsorshipController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','getResearchers'),
				'users'=>array('@'),
			),
	...
	public function actionGetResearchers() {
		if (Yii::app()->request->isAjaxRequest&&!empty($_GET['term'])) {
			$sql = 'SELECT u.id, CONCAT(p.last_name1," ",p.last_name2," ",r.names)) as label FROM users AS u JOIN persons AS p on p.id_user = u.id WHERE p.names LIKE :qterm ';
