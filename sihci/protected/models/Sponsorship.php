...
class Sponsorship extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id_user_sponsorer, id_user_researcher, state', 'numerical', 'integerOnly'=>true),
			array('project_name', 'length', 'max'=>45),
			array('description, keywords', 'length', 'max'=>150),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_user_sponsorer, id_user_researcher, project_name, description, keywords, state', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'id_user_sponsorer' => 'Id User Sponsorer',
			'id_user_researcher' => 'Id User Researcher',
